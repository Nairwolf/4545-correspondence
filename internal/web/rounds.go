...
// Lichess call — so this file needs no river client. The price: a draft
// generated here schedules no publish-round job of its own, so the
// hourly publish-round-sweep publishes it, up to an hour after its
// window ends (the same as one generated by `ic generate-round`). Only
// the weekly scheduled job's drafts publish when their window ends.
//
